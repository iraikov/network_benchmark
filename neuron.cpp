...
Neuron::Neuron(Neuron **table_,Ncq *queue_,double Vinit, double Vrinit, double Vtinit,double Elinit,double Eeinit, double Eiinit, double geinit, double giinit, double tausinit, double tauminit ,double dgiinit, double dgeinit, bool inhibiteur_,int id_)
{
  V = Vinit;
  El = Elinit;
...
void Neuron::ReceivePulse(int sender, double t)
{
...
void Neuron::pulse()
{
  ...
  for (i=0;i<(int)sources.size();i++)
    { sources[i]->ReceivePulse(id,time); }
...
Spike *Neuron::update_spike(bool exact)
{
  double g_star;
  double V_g_star;
